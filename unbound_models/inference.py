...
from unbound_models.model_utils import fix_tokenizer
...
def load_trained_lora_model(
    ...
    if binding is None:
        binding = BINDING_BUILDERS[cfg.binding_builder]()
    ...
    binding_non_lora_trainables = torch.load(
        os.path.join(model_lora_path, "binding_non_lora_trainables.bin"),
        map_location="cpu",
    )
    model.binding.load_state_dict(binding_non_lora_trainables)
